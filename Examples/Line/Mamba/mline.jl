...
old = pwd()
path = @windows ? "\\Examples\\Line\\Mamba" : "/Examples/Line/Mamba"
...
draw(model)
isfile("lineDAG.dot") && rm("lineDAG.dot")
...
isfile("mlinesummaryplot.svg") && rm("mlinesummaryplot.svg")
isfile("mlineautocormeanplot.svg") && rm("mlineautocormeanplot.svg")
isfile("mlinesummaryplot.pdf") && rm("mlinesummaryplot.pdf")
isfile("mlineautocormeanplot.pdf") && rm("mlineautocormeanplot.pdf")

## Default summary plot (trace and density plots)
p = plot(sim, legend=true)

## Write plot to file
draw(p, filename="mlinesummaryplot.svg")
draw(p, filename="mlinesummaryplot", fmt=:pdf)

## Autocorrelation and running mean plots
p = [plot(sim, :autocor) plot(sim1, :mean, legend=true)].'
draw(p, nrow=3, ncol=2, filename="mlineautocormeanplot.svg")
